...
public class VirtuosoTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            byte utf8data[] =
                { (byte) 0xd0, (byte) 0xbf, (byte) 0xd1, (byte) 0x80, (byte) 0xd0, (byte) 0xb8, (byte) 0xd0,
            ...
            if (results.length > 0) {
                ...
                if (!((results[0][0] instanceof BNode) && (results[0][1] instanceof URI) && (results[0][2] instanceof Literal))) {
                    log("TEST 1e Value types: "
                        + (ok && (results[0][0] == null) ? null : results[0][0].getClass().getName()) + ", "
                        + ((results[0][1] == null) ? null : results[0][1].getClass().getName()) + ", "
                        + ((results[0][2] == null) ? null : results[0][2].getClass().getName())); // should
                }
                endTest((ok && (results[0][0] instanceof BNode) && (results[0][1] instanceof URI) && (results[0][2] instanceof Literal))); // should
                                                                                                                                           // return
                                                                                                                                           // true
            }
            ...
            while (statements.hasNext()) {
                ...
                // System.out.println("Statement found: (" + st.getSubject() + " " + st.getPredicate() + " " +
                // st.getObject() + ")");
            }
            ...
            try {
                ...
                query =
                    "CONSTRUCT {?s <http://mso.monrai.com/mlo/handle> ?o} FROM <" + context
            ...
            try {
                ...
                // while(it.hasNext()) {
                // Statement st = it.next();
                // if( !st.getPredicate().stringValue().equals("http://mso.monrai.com/mlo/handle")) statementFound =
                // false;
                // }
            } catch (Exception e) {
                System.out.println("Error[" + e + "]");
    ...
    public static void test(String args[]) {
        try {
            ...
            stmt.execute("insert into graph <gr> " + "{ <aa> <bb> \"cc\" . <xx> <yy> <zz> . "
                    ...
                    for (int i = 1; i <= data.getColumnCount(); i++) {
                        ...
                        } else if (o instanceof VirtuosoExtendedString) // String representing an IRI
                        {
                            ...
                            if (vs.iriType == VirtuosoExtendedString.IRI)
                                ...
                                // not reached atm, literals are String or RdfBox
                                System.out.println("\"" + vs.str + "\"");
                        } else if (stmt.getResultSet().wasNull())
                            System.out.println("NULL\t");
                        else
                            System.out.println(s + " (No extended type availible)\t");
                    }
                    System.out.println("---");
                }
            ...
            // System.out.println("---");
            // VirtuosoRdfBox rb = new VirtuosoRdfBox (connection, "Some literal with many symbols over 20", null,
            // "cz");
            // System.out.println (rb.rb_box + " lang=" + rb.getLang() + " type=" + rb.getType() + " ro_id=" +
            // rb.rb_ro_id );
